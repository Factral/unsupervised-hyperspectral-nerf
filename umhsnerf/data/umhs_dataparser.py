...
def get_train_eval_split_filename(image_filenames: List) -> Tuple[np.ndarray, np.ndarray]:
    ...
    basenames = [os.path.basename(os.path.dirname(image_filename)) for image_filename in image_filenames]
...
@dataclass
class UMHSDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: UMHSDataParser)
...
class UMHSDataParser(DataParser):
    """Nerfstudio DatasetParser"""

    config: UMHSDataParserConfig
    downscale_factor: Optional[int] = None
